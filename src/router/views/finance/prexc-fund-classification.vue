...
export default {
    ...
    created() {
        ...
        this.tableBusy = true;
        axios
            .get("FundCluster/GetAllFundClusters")
            .then((res) => {
    ...
    methods: {
        ...
        onCreate(){
            ...
            if (this.$v.$invalid) {
                ...
                if(this.form.id > 0){
                    axios.put(`FundCluster/${this.form.id}`, this.form)
                    .then((res) => {
                        this.tableData.splice(this.indexSelected, 1, res.data)
                        this.$bvModal.hide('modal-standard');
                        this.onReset();
                        this.alert = { type: 'success', message: 'Successfully updated!' }
                    })
                    .catch((err) => {
                        console.log(err);
                        this.alert = { type: 'danger', message: 'Something went wrong!' }
                    ...
                    axios.post('FundCluster/CreateFundCluster', this.form)
                    .then((res) => {
                        this.tableData.push(res.data);
                        this.$bvModal.hide('modal-standard');
                        this.onReset();
                        this.alert = { type: 'success', message: 'Successfully created!' }
                    })
                    .catch((err) => {
                        console.log(err);
                        this.alert = { type: 'danger', message: 'Something went wrong!' }
        ...
        deleteItem(props){
            ...
            Swal.fire({
                ...
                if (result.value) {
                     axios
                        .delete(`FundCluster/${props.item.id}`)
                        .then((res) => {
                            if(res.status == 204){
                                this.tableData.splice(index, 1);
                                this.alert = { type: 'success', message: 'Successfully deleted' }
                            }
                        })
                        .catch((err) => {
                            console.log(err);
...
<template>
  <Layout>
    <PageHeader :title="title" :items="items" />
        <div class="row">
            <div class="col-12">
                <b-alert :show="alert.type === 'success'" dismissible @dismissed="alert.type = ''" variant="success">
                    <i class="mdi mdi-check-all me-2"></i>{{alert.message}}
                </b-alert>
                <b-alert :show="alert.type === 'danger'" dismissible @dismissed="alert.type = ''" variant="danger">
                    <i class="mdi mdi-block-helper me-2"></i>{{alert.message}}
                </b-alert>
                <b-alert :show="alert.type === 'warning'" dismissible @dismissed="alert.type = ''" variant="warning">
                ...
                <div class="card border-4 border-top border-start-0 border-end-0  border-primary">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <button type="button" @click="addFC()" class="btn btn-md btn-info d-flex align-items-center"><i class="bx bx-plus fs-6"></i>Create New</button>
                            <b-modal id="modal-standard" :title="modalTitle" size="md" title-class="font-18" hide-footer>
                                <form @submit.prevent="onCreate" class="needs-validation" ref="fcForm">
                                    <div class="mb-3">
                                        ...
                                        <input id="cluster" type="text" v-model="form.cluster" placeholder="Enter Cluster..." class="form-control" 
                                        :class="{ 'is-invalid': submitted && $v.form.cluster.$error }" 
                                        />
                                        <div v-if="submitted && $v.form.cluster.$error" class="invalid-feedback" >
                                            <span v-if="!$v.form.cluster.required">This value is required.</span>
                                        </div>
                                    </div>
                                    <div class="mb-3">
                                        <label for="uacs">UACS </label>
                                        <input id="uacs" type="text" v-model="form.uacs" placeholder="Enter UACS..." class="form-control" 
                                        :class="{ 'is-invalid': submitted && $v.form.uacs.$error }" />
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input id="shortName" type="text" v-model="form.shortName" placeholder="Enter Short Name..." class="form-control" 
                                        :class="{ 'is-invalid': submitted && $v.form.shortName.$error }" />
                                        <div v-if="submitted && $v.form.shortName.$error" class="invalid-feedback" >
                                            <span v-if="!$v.form.shortName.required">This value is required.</span>
                                        </div>
                                    </div>
