...
export default {
  ...
  methods: {
    ...
    async fetchList() {
      ...
      const posted = await this.$store.dispatch("rfppayment/getAllPosted", this.cycle);
      this.posted = posted.data;
      const pending = await this.$store.dispatch("rfppayment/getAllPending", this.cycle);
      this.pending = pending.data;
    },
    async updateAvailable(record) {
      if (record.action === "add") {
        let index = this.available.findIndex((item) => item.id == record.deliveryUnitBudgetItemId);
      ...
      if (record.action === "delete") {
        ...
        let index = this.pending.findIndex((item) => item.id == record.data.id);
        this.pending.splice(index, 1);
      }
      if (record.action === "update") {
        let index = this.available.findIndex((item) => item.id == record.deliveryUnitBudgetItemId);
        this.available[index].available = record.balance;
        let pendingIndex = this.pending.findIndex((item) => item.id == record.response.id);
    ...
    <b-tabs content-class="pt-3 pb-3 text-muted">
      <b-tab>
        <template v-slot:title>
          ...
          <span class="d-none d-sm-inline-block">Available Items</span>
        </template>
        <RequestForPayments
          :records="available"
      ...
      <b-tab>
        ...
        <RequestedForPayment :records="pending" @postPayment="postPayment" />
      </b-tab>
      <b-tab>
        <template v-slot:title>
          <span class="d-inline-block d-sm-none">
        ...
        <RequestedForPayment :records="posted" />
      </b-tab>
    </b-tabs>
  </Layout>
</template>
