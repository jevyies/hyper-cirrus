...
export default {
    ...
    created() {
        ...
        this.tableBusy = true;
        this.$store
            .dispatch("campus/GetSingleCampus", this.$route.params.id)
            .then((res) => {
                this.title = res.data.shortName + " Offices";
                this.items[2].href += res.data.agencyId;
            })
            .catch(() => {
                this.showToast("Something went wrong!", "error");
            });
        this.$store
            .dispatch("office/GetOfficeByCampus", this.$route.params.id)
            .then((res) => {
    ...
    methods: {
        ...
        onCreate() {
            ...
            if (this.$v.$invalid) {
                ...
                if (this.form.id > 0) {
                    this.$store
                        .dispatch("office/UpdateOffice", this.form)
                        .then((res) => {
                            if(res.data.error){
                    ...
                    this.$store
                        .dispatch("office/CreateOffice", this.form)
                        .then((res) => {
                            if(res.data.error){
                                return this.showToast(res.data.errorMessage, "error");
                            }
        ...
        deleteItem(props) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    this.$store
                        .dispatch("office/DeleteOffice", props.item.id)
                        .then((res) => {
                            if (res.status == 204) {
        ...
        viewDU(id) {
            this.$router.push(`/administration/delivery-unit/${id}`);
        },
    },
};
</script>
