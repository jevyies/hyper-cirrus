...
export default {
    ...
    methods: {
        ...
        addExam(){
            this.onReset();
            this.modalTitle = 'Create New Examination';
            this.$bvModal.show('e-modal');
        },
        onReset(){
            this.form = {
                id: 0,
        ...
        onCreate(){
            ...
            if (this.$v.$invalid) {
                ...
                if(this.form.id > 0){
                    this.$store.dispatch('examination/UpdateExamination', this.form)
                    .then((res) => {
                    ...
                    this.$store.dispatch('examination/CreateExamination', this.form)
                    .then((res) => {
        ...
        deleteItem(props){
            ...
            Swal.fire({
                ...
                if (result.value) {
                    this.$store.dispatch('examination/DeleteExamination', props.item.id)
                    .then((res) => {
                        if(res.status == 204){
        ...
        dropData(data){
            ...
            this.$bvModal.hide('examination-modal');
        }
    },
    created() {
		this.$store.dispatch('examination/GetExamination').then((res) => {
...
<template>
    <b-modal id="examination-modal" title="Examination List" size="lg" title-class="font-18" hide-footer no-close-on-backdrop>
        <div class="row mt-1">
            ...
            <div class="col-sm-12 col-md-6">
                <div class="d-flex justify-content-end align-items-center">
                    <button type="button" @click="addExam()" class="btn btn-sm btn-info d-flex align-items-center"><i class="bx bx-plus fs-6"></i>Create New</button>
                    <b-modal id="e-modal" :title="modalTitle" size="md" title-class="font-18" hide-footer no-close-on-backdrop>
                        <form @submit.prevent="onCreate" class="needs-validation" ref="fcForm">
                            <div class="mb-3">
                                <label for="examName">Examination Name </label>
                                <input autocomplete="off" id="examName" type="text" v-model="form.examName" placeholder="Enter Examination Name..." class="form-control" 
                                :class="{ 'is-invalid': submitted && $v.form.examName.$error }" />
                                <div v-if="submitted && $v.form.examName.$error" class="invalid-feedback" >
                                    <span v-if="!$v.form.examName.required">This value is required.</span>
                                </div>
                            </div>
