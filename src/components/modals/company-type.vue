...
export default {
    ...
    methods: {
        ...
        addCt(){
            this.onReset();
            this.modalTitle = 'Create New Company Type';
            this.$bvModal.show('ct-modal');
        },
        onReset(){
            this.form = {
                id: 0,
        ...
        onCreate(){
            ...
            if (this.$v.$invalid) {
                ...
                if(this.form.id > 0){
                    this.$store.dispatch('companytype/UpdateCompanyType', this.form)
                    .then((res) => {
                    ...
                    this.$store.dispatch('companytype/CreateCompanyType', this.form)
                    .then((res) => {
        ...
        updateItem(props){
            ...
            this.modalTitle = 'Update Company Type';
            this.$bvModal.show('ct-modal');
        },
        deleteItem(props){
            ...
            Swal.fire({
                ...
                if (result.value) {
                    await this.$store.dispatch('companytype/DeleteCompanyType', props.item.id)
                    .then((res) => {
                        if(res.status == 204){
        ...
        dropData(data){
            ...
            this.$bvModal.hide('company-type-modal');
        }
    },
    created() {
        this.tableBusy = true;
		this.$store.dispatch('companytype/GetCompanyType')
            .then((res) => {
...
<template>
    <b-modal id="company-type-modal" title="Company Type List" size="lg" title-class="font-18" hide-footer no-close-on-backdrop>
        <div class="row mt-1">
            ...
            <div class="col-sm-12 col-md-6">
                <div class="d-flex justify-content-end align-items-center">
                    <button type="button" @click="addCt()" class="btn btn-sm btn-info d-flex align-items-center"><i class="bx bx-plus fs-6"></i>Create New</button>
                    <b-modal id="ct-modal" :title="modalTitle" size="md" title-class="font-18" hide-footer no-close-on-backdrop>
                        <form @submit.prevent="onCreate" class="needs-validation" ref="fcForm">
                            <div class="mb-3">
                                <label for="type">Type </label>
                                <input autocomplete="off" id="type" type="text" v-model="form.type" placeholder="Enter Company Type..." class="form-control" 
