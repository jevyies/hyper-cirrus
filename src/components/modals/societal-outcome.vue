...
export default {
    ...
    methods: {
        ...
        addPc() {
            this.onReset();
            this.modalTitle = "Create New Societal Outcome";
            this.$bvModal.show("so-modal");
        },
        onReset() {
            this.form = {
                id: 0,
        ...
        onCreate() {
            ...
            if (this.$v.$invalid) {
                ...
                if (this.form.id > 0) {
                    this.$store
                        .dispatch(
                            "societaloutcome/UpdateSocietalOutcome",
                            this.form
                        )
                        .then((res) => {
                    ...
                    this.$store
                        .dispatch(
                            "societaloutcome/CreateSocietalOutcome",
                            this.form
                        )
                        .then((res) => {
        ...
        updateItem(props) {
            ...
            this.modalTitle = "Update Societal Outcome";
            this.$bvModal.show("so-modal");
        },
        deleteItem(props) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    this.$store
                        .dispatch(
                            "societaloutcome/DeleteSocietalOutcome",
                            props.item.id
                        )
                        .then((res) => {
                            if (res.status == 204) {
        ...
        dropData(data) {
            ...
            this.$bvModal.hide("societal-outcome-modal");
        },
        getData() {
            this.tableBusy = true;
            this.$store
                .dispatch("societaloutcome/GetSocietalOutcome")
                .then((res) => {
...
<template>
    <b-modal
        id="societal-outcome-modal"
        title="Societal Outcome List"
        size="lg"
        title-class="font-18"
        hide-footer
